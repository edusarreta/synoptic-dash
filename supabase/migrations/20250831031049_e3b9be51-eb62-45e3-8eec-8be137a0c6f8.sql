-- Complete database schema for analytics dashboard system
...
CREATE TABLE IF NOT EXISTS public.user_grants (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(org_id, COALESCE(workspace_id, '00000000-0000-0000-0000-000000000000'::uuid), user_id, perm_code)
);
...
CREATE OR REPLACE FUNCTION public.has_permission(_user_id uuid, _org_id uuid, _workspace_id uuid, _perm_code text)
RETURNS boolean
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
  WITH role_perms AS (
    ...
    JOIN public.profiles p ON p.org_id = rp.org_id
    WHERE p.id = _user_id 
      AND rp.org_id = _org_id 
      AND rp.role = p.role 
      AND rp.perm_code = _perm_code
  ),
  user_grants AS (
    SELECT effect FROM public.user_grants ug
    WHERE ug.user_id = _user_id 
      AND ug.org_id = _org_id
      AND (ug.workspace_id = _workspace_id OR ug.workspace_id IS NULL)
      AND ug.perm_code = _perm_code
    ORDER BY CASE WHEN effect = 'DENY' THEN 0 ELSE 1 END
    LIMIT 1
  )
  SELECT CASE 
    WHEN (SELECT effect FROM user_grants) = 'DENY' THEN false
    WHEN (SELECT effect FROM user_grants) = 'ALLOW' THEN true
