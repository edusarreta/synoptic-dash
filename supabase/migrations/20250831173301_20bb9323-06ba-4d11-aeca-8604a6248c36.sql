...
BEGIN
    ...
    IF master_user_id IS NOT NULL AND master_org_id IS NOT NULL THEN
        RAISE NOTICE 'Found MASTER user % with org %', master_user_id, master_org_id;
        ...
        RAISE NOTICE 'Bootstrap completed! MASTER user % now has % permissions', master_user_id, perm_count;
    ELSE
