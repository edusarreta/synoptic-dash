...
export default function ChartBuilder({ editChartId }: ChartBuilderProps = {}) {
  ...
  const loadChartForEditing = async (chartId: string) => {
    ...
    try {
      ...
      if (chart) {
        ...
        // Restore selectedFields with aggregation information
        if (chart.chart_config && typeof chart.chart_config === 'object' && 'selectedFields' in chart.chart_config) {
          const config = chart.chart_config as any;
          if (Array.isArray(config.selectedFields)) {
            console.log('✅ Restoring selectedFields with aggregations:', config.selectedFields);
            setSelectedFields(config.selectedFields);
            
            // Also set selected tables based on the fields
            const tablesFromFields = [...new Set(config.selectedFields.map((f: any) => f.tableName).filter(Boolean))] as string[];
            console.log('✅ Setting tables from fields:', tablesFromFields);
            setSelectedTables(tablesFromFields);
          }
        }
  ...
  const handleFieldAggregationChange = (field: SelectedField, aggregation: 'sum' | 'count' | 'avg' | 'min' | 'max' | 'count_distinct') => {
    console.log('🔢 Changing aggregation for field:', field.columnName, 'to:', aggregation);
    setSelectedFields(prev => {
      ...
      console.log('🔢 Updated selectedFields:', updated);
      return updated;
    });
  };
  ...
  const handleSaveChart = async () => {
    ...
    try {
      ...
      const chartConfig = {
        ...
        })), // Save complete field information including aggregations
        filterableColumns: filterableColumns // NOVA FEATURE: Colunas disponíveis para filtro
      };
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            {editChartId ? 'Editar Gráfico' : 'Chart Builder'}
          </h1>
          <p className="text-muted-foreground mt-1">
            {editChartId ? 'Modifique seu gráfico e configure filtros personalizados' : 'Create interactive charts and visualizations from your data - Looker Studio style'}
          </p>
        </div>
                ...
                <CardContent>
                    ...
                    {availableColumns.map((column) => (
                      <div key={column} className="flex items-center space-x-3">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {filterableColumns.includes(column) ? 'Filtrável' : 'Não filtrável'}
                        </span>
                      </div>
                    ))}
                  </div>
                  {filterableColumns.length > 0 && (
                    <div className="mt-4 p-3 bg-muted/50 rounded-md">
                      <p className="text-xs text-muted-foreground">
                        <strong>{filterableColumns.length}</strong> coluna(s) selecionada(s) para filtros dinâmicos
                      </p>
                    </div>
                  )}
                </CardContent>
